...
public class DataResourceDoFn extends DoFn<String, DataResourceRecord> {
	...
	@Setup
	public void setup() {
		dataResourceService = ClientFactory.createRetrofitClient("https://collections.ala.org.au/ws/",
	...
	public void processElement(ProcessContext context) {
		try {
			...
			DataResourceRecord dataResourceRecord = DataResourceRecord.newBuilder().setId(dataResourceId)
					.setDataResourceId(dataResourceId).setName(response.getName()).build();
			context.output(dataResourceRecord);
		} catch (IOException ex) {
			LOG.error("Error contacting collections web service", ex);
		}
	}
}
